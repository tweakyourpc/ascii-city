...
export class Panel {
  ...
  _lines(cam, world) {
    ...
    if (hit.kind === 'building') {
      ...
      kv('Web', (t.website || t['contact:website'])
        ? hostOf(t.website || t['contact:website']) : null);
  ...
  draw(screen, cam, world) {
    ...
    rows.push(['', null]);
    for (const [k, v] of L) {
      rows.push([k ? `${k.padEnd(10)}${v}` : v, k ? VALUE : BODY, k]);
    }
    if (L.body) {
      rows.push(['', null]);
      for (const line of L.body) rows.push([line, BODY]);
    }
    rows.push(['', null]);
